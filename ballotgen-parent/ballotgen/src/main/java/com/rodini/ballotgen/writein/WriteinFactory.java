...
import com.rodini.ballotgen.Initialize;
...
public class WriteinFactory {
	...
	static void processLine(String candName, String contestTitle,
		...
		List<String> muniNoList = Arrays.stream(precinctNos)
				...
				.collect(toList());
		Writein writein = new Writein(candName, contestTitle, zoneNo, muniNoList);
	...
	static void processData(int lineNo, String[] fields) {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			case 1:
				// Contest name
				contestTitle = field;
				if (contestTitle.isBlank() || contestTitle.length() < 3) {
					logger.error(String.format("CSV line #%d write-in contest name %s has error", lineNo, contestTitle));
					return;
				}
				break;
			case 2:
		...
		processLine(candName, contestTitle, zoneNo, precinctNos);
	}
	...
	private static int[] validateZoneOwnership(int lineNo, int zoneNo, int[] precinctNos) {
		...
		for (int i = 0; i < precinctNos.length; i ++) {
			...
			if (!zoneOwnsPrecinct(zoneStr, precinctStr)) {
	...
	// Get the precinctZonesMap
	// Note - must be called before process() method.
	public static Map<String, Zone> getPrecinctToZones() {
		return precinctToZoneMap;
	}
	...
	public static boolean zoneOwnsPrecinct(String zoneNo, String precinctNo) {
		Zone zone = precinctToZoneMap.get(precinctNo);
		if (zone == null) {
			return false;
		}
