...
import static com.rodini.ballotutils.ElectionType.PRIMARY;
...
import com.rodini.ballotprocessor.model.GeneralCandidate;
import com.rodini.ballotprocessor.model.PrimaryCandidate;
...
public class CandidateFactory {
	...
	private final String contestTitle;	 // title of the contest / office
	...
	// See properties file.
//	private static final List<String> titlesOfTicketContests = List.of(
//			"GOVERNOR AND LIEUTENANT GOVERNOR",
//			"PRESIDENT AND VICE-PRESIDENT"
//			);
//	private static final List<String> titlesOfTicketContests = 
//			Arrays.asList(Utils.getPropValue(ballotGenProps, PROP_TICKET_CONTEST_TITLES).split(","));
	
//	private static final List<String> titlesOfLocalContests = List.of(
...
//			);
	...
	/**
	 * Constructor
	 * @param contestTitle contest that the candidates are vying for.
	 ...
	 */
	public CandidateFactory(String contestTitle, String candidatesText, ElectionType type, Party endorsedParty) {
	...
	private void processCandidatesText() {
		if (elecType == PRIMARY) {
			if (isLocalContest(contestTitle)) {
			...
			if (isTicketContest(contestTitle)) {
	 ...
	 * @param contestTitle office name.
	 * @return true => yes
	 */
	private boolean isLocalContest(String contestTitle) {
		boolean local = false;
		contestTitle = normalizeContestName(contestTitle);
		local = contestTitleMatch(contestTitle, Initialize.titlesOfLocalContests) && 
				!contestTitleExceptionMatch(contestTitle, Initialize.titlesOfLocalContestsExceptions) ;
		return local;
	}
	/**
	 ...
	 * @return true => yes
	 */
	private boolean isTicketContest(String contestTitle) {
		boolean ticket = false;
		contestTitle = normalizeContestName(contestTitle);
		ticket = contestTitleMatch(contestTitle, Initialize.titlesOfTicketContests);
		return ticket;
	}
	/**
	...
	/**
	 * contestTitleMatch determines if the normalized contest name matches
	 ...
	 * @return
	 */
	private boolean contestTitleMatch(String normalName, List<String> prefixNames)  {
		boolean match = false;
	...
	/**
	 * contestTitleExceptionMatch determines if the contest name belongs to one
	 ...
	 * @return
	 */
	private boolean contestTitleExceptionMatch(String normalName, List<String> exceptionNames) {
		boolean match = false;
