...
public class GenerateBallotSummary {
	...
	static final String SUMMARY_FILE_NAME = "ballot-summary.txt";
     ...
     */
	public static void generate(Election election) {
		
		List<Ballot> ballots = election.getBallots();
		Path summaryPath = Paths.get(BallotProcessor.storeDirPath).getParent();
		String summaryFile = summaryPath.toString();
		try (PrintWriter pw = new PrintWriter(new File(summaryFile + File.separator + SUMMARY_FILE_NAME))) {
			generateHeading(pw, election, ballots.size());
	 ...
	 */
	static void generateHeading(Writer pw, Election election, int ballotCount) throws IOException {
		// Identify the report by info within the VS specimen.
		String electionName = election.getTitle();
		String line = String.format("Summary Report: %s\n", electionName);
		System.out.print(line);
		pw.write(line);
		String type = election.getElectionType().toString();
		line = String.format("Election type: %s\n", type);
		System.out.print(line);
		pw.write(line);	
		line = "Precinct count: " + ballotCount + "\n";
		System.out.print(line);
		pw.write(line);
	}
	/** 
	...
	static void generateUniqueBallotSection(Writer pw, List<Ballot> ballots) throws IOException {
		...
		String line = String.format("Unique ballot count: %d\n", uniqueKeys.size());
		System.out.print(line);
	...
	static String getUniqueString(Ballot ballot) {
		...
		String contestsString = ballot.getContests().stream().map(Contest::getTitle).collect(joining(""));
...
//		}
		String refsString    = ballot.getReferendums().stream().map(Referendum::getTitle).collect(joining(""));
		String retsString    = ballot.getRetentions().stream().map(Retention::getTitle).collect(joining(""));
	...
	static void generateReferendumSection(Writer pw, List<Ballot> ballots) throws IOException {
		...
		for (Ballot ballot: ballots) {
			...
			String refString = ballot.getReferendums().stream().map(Referendum::getTitle).collect(joining(""));
