...
class TestPrecinctFactory {
	...
	void testOnePrecinct() {
		...
		Precinct precinct1 = PrecinctFactory.findOrCreate(root, precinctNo, precinctName, precinctZoneNo);
		Map<String, Precinct> map = root.getPrecinctNoPrecinctMap();
	...
	void testTwoPrecincts() {
		...
		Precinct precinct1 = PrecinctFactory.findOrCreate(root, precinctNo1, precinctName1, precinctZoneNo1);
		String precinctNo2 = "010";
		String precinctName2 = "AVONDALE";		
		String precinctZoneNo2 = "08";
		Precinct precinct2 = PrecinctFactory.findOrCreate(root, precinctNo2, precinctName2, precinctZoneNo2);
		Map<String, Precinct> map = root.getPrecinctNoPrecinctMap();
		Set<String> keys = map.keySet();
	...
	void testDuplicatePrecincts() {
		...
		String precinctZoneNo1 = "03";
		Precinct precinct1 = PrecinctFactory.findOrCreate(root, precinctNo1, precinctName1, precinctZoneNo1);
		Precinct precinct2 = PrecinctFactory.findOrCreate(root, precinctNo1, precinctName1, precinctZoneNo1);
		Map<String, Precinct> map = root.getPrecinctNoPrecinctMap();
		Set<String> keys = map.keySet();
		assertEquals(1, keys.size());
		assertEquals(precinct1, map.get(precinctNo1));	
