...
public class ZoneDataProcessor {
	...
	static void processLine(DataRoot root, String zoneNo, String zoneName, String zoneLogoPath, String zoneUrl, String zoneChunkPath) {
		...
		Zone zone = ZoneFactory.findOrCreate(root, normalZoneNo, zoneName, zoneLogoPath, zoneUrl, zoneChunkPath);
	}

	// Process data from single CSV line.
	// Format: zone no. (number), zone name (text), zone logo (image), zone website (url), zone chunk (docx)
	static void processData(DataRoot root, int lineNo, String[] fields) {
		...
		processLine(root, field0, field1, field2, field3, field4);
	}
	 ...
	 */
	static void processZonesText(DataRoot root, String zonesText) {
