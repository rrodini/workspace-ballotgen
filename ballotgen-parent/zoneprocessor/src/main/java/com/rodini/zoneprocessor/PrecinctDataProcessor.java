...
public class PrecinctDataProcessor {
	...
	static void processLine(DataRoot root, String precinctNo, String precinctName, String zoneNo) {
		...
		logger.debug(String.format("precinctNo: %s precinctName: %s zoneNo: %s", normalPrecinctNo, precinctName,
				normalZoneNo));
		Precinct precinct = PrecinctFactory.findOrCreate(root, normalPrecinctNo, precinctName, normalZoneNo);
	}
	...
	static void processData(DataRoot root, int lineNo, String[] fields) {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			case 2:
				...
				processLine(root, field0, field1, field2);
				break;
			}
		}
	}
	 ...
	 */
	public static void processPrecinctsText(DataRoot root, String precinctsText) {
